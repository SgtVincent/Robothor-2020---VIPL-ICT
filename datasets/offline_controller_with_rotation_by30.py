...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    # Since in the 30-degree case. the topology of original graph file (0.125-grid graph)
    # is not the logical topology of reachable positions in scene
    # we first disable the correction check with neighbors
    # TODO: create logical topology for all scenes
    def step(self, action, raise_for_failure=False, graph_check=False):
        ...
        if self.visualize and next_state is not None:
            ...
            if (
                ...
                self.controller.teleport_to_state(self.state)
        # Since the gridSize has been
        if graph_check: # check whether next_state is accessible by check node neighbors in graph.json
            if next_state is not None:
                ...
                if next_state_key in neighbors:
                    ...
                    return event
        # TODO: Check intergrity
        else: # check whether next_state is accessible by check if next_state in self.images
            if next_state in self.images:
                next_state_key = str(next_state)
                self.state = self.get_state_from_str(
                    *[float(x) for x in next_state_key.split("|")]
                )
