...
if __name__ == "__main__":
    ...
    controller = Controller(**wrong_case_config)
    event = controller.step(action="Teleport", x=-1.50, y=1.00, z=1.00)
    print("Initial state: (x, y, z, rotation, horizon)={}".format(get_state_from_evenet(event)))
    event = controller.step(action="Teleport", x=-1.75, y=1.00, z=1.00)
